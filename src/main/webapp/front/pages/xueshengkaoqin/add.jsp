...
<body style="background: #fff; ">
    ...
    <!-- 当前表的 -->
                ...
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    ...
                    <div class="layui-input-block">
                        <select name="xueshengkaoqinTypes" id="xueshengkaoqinTypes" lay-filter="xueshengkaoqinTypes" :disabled="ro.xueshengkaoqinTypes">
                            <option v-for="(item,index) in xueshengkaoqinTypesList" v-bind:key="index" :value="item.codeIndex" :selected="detail.xueshengkaoqinTypes == item.codeIndex?true:false" :key="item.codeIndex">{{ item.indexName }}</option>
                        </select>
                    </div>
                </div>
            <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                ...
                <div class="layui-input-block">
                    <textarea class="main_borderColor layui-input" style="width: 100%;height: 100px;border-radius: 5px;padding: 10px" v-model="detail.xueshengkaoqinContent" lay-verify="required" :readonly="ro.xueshengkaoqinContent" name="xueshengkaoqinContent" id="xueshengkaoqinContent" autocomplete="off"></textarea>
                </div>
            </div>
                    <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    <label  :style='{"width":"auto","borderColor": "rgba(255, 255, 255, 0)","padding":"0 12px 0 0","backgroundColor":"rgba(255, 255, 255, 0)","fontSize":"15px","color":"#333","textAlign":"left"}' class="layui-form-label">
                    ...
                    <div class="layui-input-block">
                        <input class="layui-input main_borderColor" :style='{"padding":"0 12px","boxShadow":"0 0 0px rgba(160, 67, 26, 1)","backgroundColor":"#fff","color":"rgba(135, 206, 250, 1)","borderRadius":"4px","textAlign":"left","borderWidth":"1px","fontSize":"15px","borderStyle":"solid","height":"40px"}'
                           type="text" :readonly="ro.kaoqinTime" lay-verify="required" name="kaoqinTime" id="kaoqinTime" autocomplete="off">
                    </div>
                </div>
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
    ...
    <script>
        ...
        layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
           ...
           // 上传文件
            ...
            //修改
            if(http.getParam('xueshengkaoqinId')){
                http.request(`xueshengkaoqin/info/${http.getParam('xueshengkaoqinId')}`, 'get', {}, function(data) {
                    vue.detail = data.data
                    jquery("#kaoqinTime").val(vue.detail.kaoqinTime)
            ...
            form.on('submit(thisSubmit)', function (data) {
                ...
                if(http.getParam('xueshengkaoqinId')){
                    data["id"]= http.getParam('xueshengkaoqinId')
                }
                // 提交数据
                http.requestJson(`xueshengkaoqin/${http.getParam("xueshengkaoqinId")?"update":"add"}`, 'post', data, function (res) {
                    if(res.code == 0){
                        layer.msg('提交成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            vue.jump("../xueshengkaoqin/list.jsp");
                                http.remove('xueshengkaoqinId')
                            // back();
                        });
                    }else{
        ...
        function dateTimePick(){
            ...
            */
            // 考勤日期的开始时间和结束时间的效验及格式
            var kaoqinTime = layui.laydate.render({
                elem: '#kaoqinTime'
	            ,type: 'date'
                ,btns: ['clear', 'confirm']
                ,min:myDate.toLocaleString()
                ,change: function(value, date, endDate){
                    vue.detail.kaoqinTime = value;
                }
            });
        }
       ...
       function xueshengkaoqinTypesSelect() {
           //填充下拉框选项
           layui.http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xueshengkaoqin_types", "GET", {}, (res) => {
               if(res.code == 0){
