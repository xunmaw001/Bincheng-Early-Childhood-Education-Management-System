...
<!-- 标题 -->
     <div class="index-title sub_backgroundColor" :style='{"padding":"10px","boxShadow":"0 0 2px rgba(160, 67, 26, 1)","margin":"10px auto","borderColor":"rgba(0,0,0,.3)","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","borderWidth":"0","fontSize":"20px","borderStyle":"solid","height":"auto"}'>
         <span>USER / ORDER</span><span>我的缴费</span>
     </div>
        ...
        <!-- 个人中心菜单 config.js-->
            <div class="left-container" :style='{"backgroundColor":"#fff","padding":"0","boxShadow":"0px 0px 1px rgba(181, 181, 181, 1)","margin":"0","borderColor":"rgba(191, 191, 191, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"0","borderWidth":"0 0 1px 0","width":"20%","borderStyle":"solid"}'>
                <ul class="layui-nav layui-nav-tree">
                    <li v-for="(item,index) in centerMenu" v-bind:key="index" class="layui-nav-item" :class="item.url=='../feiyong/list.jsp'?'layui-this':''">
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.feiyongTypes=="" || searchForm.feiyongTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in feiyongTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.feiyongTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
<!--                <button @click="jump('../feiyong/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加缴费信息
                </button>-->
            </div>
        ...
        <!-- 级联表 -->
                        ...
                        <td>
                            <img v-if="item.laoshiPhoto" :src="baseUrl+item.laoshiPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            <span v-else >暂无照片</span>
                        </td>
                        <td>{{item.banjiName}}</td>
                        <td>{{item.feiyongName}}</td>
                        <td>{{item.feiyongValue}}</td>
                        <td>{{item.feiyongZhuangtaiValue}}</td>
                        <td>{{item.feiyongOldMoney}}</td>
                        <td>{{item.insertTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../feiyong/add.jsp?feiyongId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
...
<%--                            </button>--%>
                            <button @click="jiaofei(item)" type="button" v-if="item.feiyongZhuangtaiTypes==2"
                                    class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                缴费
                            </button>
                            <button v-if="item.feiyongTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`feiyong/update`, 'post', {
                    id:id,
//                    feiyongTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            jiaofei(data) {
                var _this = this;
                _this.jiaofeiDate = data
                if (_this.jiaofeiDate.feiyongOldMoney > _this.jiaofeiDate.newMoney) {
                    alert("余额不足请及时充值")
                } else {
                    _this.jiaofeiDate.feiyongZhuangtaiTypes = 1
                    layui.http.requestJson(`feiyong/update`, 'post', _this.jiaofeiDate, (res) => {
                        if (res.code == 0) {
                            _this.jiaofeiDate.newMoney = _this.jiaofeiDate.newMoney - _this.jiaofeiDate
                                .feiyongOldMoney
                            _this.jiaofeiDate.id = _this.jiaofeiDate.yonghuId
                            layui.http.requestJson(`yonghu/update`, 'post', _this.jiaofeiDate, (res) => {
                                if (res.code == 0) {
                                    layer.msg('缴费成功', {
                                        time: 2000,
                                        icon: 6
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`feiyong/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('操作成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.feiyongTypes = $(this).attr('index');
            pageList();
        });
           ...
           function feiyongTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=feiyong_types", 'get', {}, function (res) {
                   if(res.code == 0){
           ...
           function feiyongZhuangtaiTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=feiyong_zhuangtai_types", 'get', {}, function (res) {
                   if(res.code == 0){
            ...
            function pageList() {
                // 获取列表数据
                http.request('feiyong/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('feiyong/list', 'get', vue.searchForm, function (res1) {
