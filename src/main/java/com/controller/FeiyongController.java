    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(feiyong !=null){
            ...
            //级联表 老师
            //级联表
            LaoshiEntity laoshi = laoshiService.selectById(feiyong.getLaoshiId());
            if(laoshi != null){
            BeanUtils.copyProperties( laoshi , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setLaoshiId(laoshi.getId());
            }
            //级联表 班级
            //级联表
            BanjiEntity banji = banjiService.selectById(feiyong.getBanjiId());
            if(banji != null){
            BeanUtils.copyProperties( banji , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
    ...
    */
    ...
    public R save(@RequestBody FeiyongEntity feiyong, HttpServletRequest request){
        ...
        Wrapper<FeiyongEntity> queryWrapper = new EntityWrapper<FeiyongEntity>()
            ...
            .eq("laoshi_id", feiyong.getLaoshiId())
            .eq("banji_id", feiyong.getBanjiId())
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            FeiyongEntity feiyongEntity = new FeiyongEntity();
//                            feiyongEntity.setYonghuId(Integer.valueOf(data.get(0)));   //学生 要改的
//                            feiyongEntity.setLaoshiId(Integer.valueOf(data.get(0)));   //老师 要改的
//                            feiyongEntity.setBanjiId(Integer.valueOf(data.get(0)));   //班级 要改的
